...
def response = WS.sendRequest(findTestObject('Activities/POST'))

WS.verifyResponseStatusCode(response, 200)

def jsonResponse = new JsonSlurper().parseText(response.getResponseBodyContent())

// Verifikasi field ada dan tipe datanya 
assertThat(jsonResponse.id).isInstanceOf(Integer)
assertThat(jsonResponse.title).isInstanceOf(String)
assertThat(jsonResponse.dueDate).isInstanceOf(String)
assertThat(jsonResponse.completed).isInstanceOf(Boolean)
